...
sumdf %>%
  ...
  labs(x = NULL, y = "Incidents per 1,000 Miles",
       subtitle = "All Incidents from 1/1/2018-12/31/2022",
       title = "5-Year Incident Rate by System & Program", tag = tag)

ggsave("napsr_plots/ir_comp.png", width = wdt, height = hgt)
```
...
ggsave("napsr_plots/dmg_comp.png", width = wdt, height = hgt)
```
...
ggsave("napsr_plots/rel_comp.png", width = wdt, height = hgt)
```
...
dfRaw <- map_dfr(years, read_xlsx, 
  ...
  rename(state = State) %>%
  mutate(eastern = state.abb[match(state,state.name)] %in% napsr  | state == "DC",
         score_per = average/3)
...
df_wide <- df_23 %>%
  ...
  mutate(year = 2023,
         ...
         eastern = state.abb[match(state,state.name)] %in% napsr| state == "DC")%>%
  bind_rows(dfRaw)%>%
...
ggplot()+
  ...
  labs(title = "NAPSR East Transparency Ratings",
       tag = tag)

plot_name = "class_hex.pdf"
ggsave(plot_name, path= "napsr_plots/", device = "pdf",
...
ggplot()+
  ...
  scale_fill_viridis(option = "G", name = "Score", end = .8, limits = c(0,33),
                     breaks = c(0,18,25,33))+
  theme_pst_map()+
  labs(title = "NAPSR East Transparency Scores",
       tag = tag)+
...
ggplot()+
  ...
  labs(title = "Average State Score by Category: NAPSR East", tag = tag)

plot_name = "score_cat_hex.pdf"
ggsave(plot_name, path= "napsr_plots/", 
       width = wdt, height = hgt)
```
...
df %>%
  ...
  labs(x = NULL, y = "Average Score",
       ...
       subtitle = "NAPSR Eastern Region Compared to All Other States")

ggsave("napsr_plots/Avg_time.png", width = wdt, height = hgt, units = "in")
```
...
df %>%
  ...
  ggplot(aes(x = year, y = score, col = nameCat, linetype = eastern))+
...
ggsave("napsr_plots/Category_Avg_time.png", width = wdt, height = hgt, units = "in")
...
df %>%
  ...
  labs(x = NULL, y = "Average Score",
       tag = tag, title = "Average Score by Category",
       subtitle = "NAPSR Eastern Region")

ggsave("napsr_plots/Category_Avg_time_onlyeast.png", width = wdt, height = hgt, units = "in")
...
df %>%
  ...
  labs(title = "State Transparency Ratings: NAPSR East",
...
plot_name = "napsr_plots/matrix_tile_avg.pdf"
ggsave(plot_name,
       width = wdt, height = hgt)
...
df %>%
  ...
  mutate(state = factor(state),
         state = fct_reorder(state, score_2023)) %>% 
  ggplot()+
  ...
  annotate(x = "District of Columbia", y = 32.5, angle = 90,
...
ggsave("napsr_plots/5_10_Year_Cleveland.pdf", width = wdt, height = hgt, units = "in")
```
