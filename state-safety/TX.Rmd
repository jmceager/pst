...
df <- read_csv(paste0(datadir, "clean/all_inc.csv")) %>%
  filter(STATE == moState, IYEAR < 2023) 
...
milesSYS <- read_csv(paste0(datadir, "clean/sys_miles.csv")) %>%
...
```{r tx sig raw and ser raw}
df %>%
  filter(SIGNIFICANT == "YES", SYS %in% c("GT", "GD", "HL"))%>%
  count(IYEAR, SYS)%>%
  ggplot(aes(x = IYEAR, y = n, col = SYS, fill = SYS))+
  geom_smooth(method = "lm", linetype = "dashed", linewidth = .66)+
  geom_line(linewidth = .9)+
  scale_fill_manual(values = pst1, name = "System", labels = c("Gas Distribution", 
                                                               "Gas Transmission",
                                                               "Hazardous Liquids"))+
  scale_color_manual(values = pst1, name = "System", labels = c("Gas Distribution", 
                                                               "Gas Transmission",
                                                               "Hazardous Liquids"))+
  scale_x_continuous(breaks = seq(2010,2022,2))+
  theme_pst()+
  theme(legend.position = c(0.7,0.45))+
    coord_cartesian(xlim = c(2010,2022.5), ylim = c(0, 80), expand = F)+
  labs(title = "Significant Incident Trends in Texas",
       subtitle = "Raw Incident Count per System",
       caption = capt,
       tag = tag,
       x = "Year", y = "Incidents")

plotName = "TX_SigIncSys.png"
plotExport = paste0(exportdir, plotName)
wdt = 8
hgt = wdt/1.62
ggsave(plotExport, width = wdt, height = hgt, units = "in")
...
df %>%
  filter(SERIOUS == "YES", SYS %in% c("GT", "GD", "HL"))%>%
  count(IYEAR, SYS)%>%
  complete(IYEAR, SYS, fill = list(n = 0))%>%
  ggplot(aes(x = IYEAR, y = n, col = SYS, fill = SYS))+
  #geom_smooth(method = "lm", linetype = "dashed", linewidth = .66)+
  ...
  scale_y_continuous(breaks = seq(0,12,3))+
    scale_x_continuous(breaks = seq(2010,2022,2))+
  coord_cartesian(xlim = c(2010,2022.5), ylim = c(0, 10), expand = F)+
  theme(legend.position = c(0.2,0.75))+
  labs(title = "Serious Incident Trends in Texas",
       subtitle = "Raw Incident Count per System",
       caption = capt,
       tag = tag,
       x = "Year", y = "Incidents")

plotName = "TX_SerIncSys.png"
plotExport = paste0(exportdir, plotName)
wdt = 8
hgt = wdt/1.62
ggsave(plotExport, width = wdt, height = hgt, units = "in")
...
df %>%
  filter(SIGNIFICANT == "YES", SYS %in% c("GT", "GD", "HL"))%>%
  count(IYEAR, SYS)%>%
  left_join(milesSYS, by = c("IYEAR", "SYS"))%>%
  mutate(rate = n / (miles/1000))%>%
  ggplot(aes(x = IYEAR, y = rate, col = SYS, fill = SYS))+
  geom_smooth(method = "lm", linetype = "dashed", linewidth = .66)+
  geom_line(linewidth = .9)+
  scale_fill_manual(values = pst1, name = "System", labels = c("Gas Distribution", 
                                                               "Gas Transmission",
  ...
  theme_pst()+
  theme(legend.position = c(0.7,0.45))+
    coord_cartesian(xlim = c(2010,2022.5),  expand = F)+
  labs(title = "Significant Incident Trends in Texas",
       subtitle = "Annual Incident Rate per System",
       caption = capt,
       tag = tag,
       x = "Year", y = "Incidents per 1,000 Miles")

plotName = "TX_SigIncSys_Rate.png"
plotExport = paste0(exportdir, plotName)
wdt = 8
hgt = wdt/1.62
ggsave(plotExport, width = wdt, height = hgt, units = "in")
...
plotName = "TX_SerIncSys_Rate.png"
plotExport = paste0(exportdir, plotName)
wdt = 8
hgt = wdt/1.62
ggsave(plotExport, width = wdt, height = hgt, units = "in")
...
df %>%
  ...
  scale_color_manual(values = pst1,
                     ...
                     labels = c(moStateL, "Adjacent (avg.)", "U.S.A. (avg.)"))+
  scale_x_continuous(labels = seq(2010,2022,2), breaks = seq(2010,2022,2))+
  scale_y_continuous(limits = c(-.001, .08),breaks = seq(0,.10,.025),
                     expand = expansion(0))+ 
...
df.release %>%
  ...
  geom_label_repel(data = filter(df.release, TOTAL_RELEASE > 2 | 
                                   SIGSER == 2 & FATAL > 1 |
                                   TOTAL_COST_CURRENT > 2.5e6),
 ...
 # annotate("text", x = ymd("2010-12-30"), y = 8, label = "ok")+
  ...
  scale_y_continuous(limits = c(-1,6.7))+
