...
df <- read_csv(paste0(datadir, "clean/all_inc.csv")) %>%
  filter(STATE == moState, IYEAR < 2024, REPORT_NUMBER != 20220042) 
...
miles_tern <- milesFull %>%
  ...
  mutate(across(GD:HL, ~replace_na(.x, 0)),
         total = GD+GT+HL,
         across(GD:HL, ~scale(.x, center = F)),
         do_col = STATE == moState)


ggplot(data = miles_tern, aes(x = GD, y = GT, z = HL, size = total, col = do_col))+
...
t.gd <- df.t %>%
  ...
  group_by(pri.name)%>%
  summarise(across(!any_of(c("pri.name", "pri.id", "SYS")), sum, na.rm = T))%>%
...
df.release %>%
  ...
  geom_label_repel(data = filter(df.release, TOTAL_RELEASE > 3 | 
                                   SIGSER == 2 & FATAL >1 |
 ...
 # annotate("text", x = ymd("2010-12-30"), y = 8, label = "ok")+
  ...
  scale_y_continuous(limits = c(-1,7), breaks = seq(-2,8,2))+
...
df.c%>%
  ...
  labs(y = NULL,
       ...
       subtitle = paste0("Selected Operators in ", moStateL, " (2010-2022)"),
       tag = tag, 
       caption = capt)+
  theme_pst()+
...
base_tiles <- get_static_tiles(location = stateShape,
                               zoom = 5,
                               style_id = "cl89e4tvy000115obef04fadi",
                               username = "jimeagah",
                               buffer_dist = 1000000,
                               scaling_factor = "2x",
                               crop = T)
...
xdist_exp <- .25*xdist
bxn <- bbox[[1]] - (xdist_exp*1.2)
bxx <- bbox[[3]] + (xdist_exp*.05)
...
ggplot()+
  geom_point()+
  scale_x_continuous(limits = c(500,1500), name = "Maximum Operating Pressure (psi)")+
  scale_y_continuous(limits = c(0,1320), breaks = seq(0,1320, 330), expand = expansion(0,0), name = "Hazard Area Radius (ft)")+
  labs(title = "Proposed hazard area radius as a function of line diameter and pressure",
       tag = tag,
       caption = "Source: C-FER Technologies")+
  theme_pst()

ggsave("plot_blank.pdf", width = 7, height = 7/1.62, units = "in")
```
