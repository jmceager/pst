...
pt <- ggplot(data = miles_tern, aes(x = GD_p, y = GT_p, z = HL_p, 
  ...
  # geom_text(data = . %>% filter(do_col),aes(label=STATE), alpha = .9)+
...
ggplot2::ggsave("NY/arrange.png",  width = wdt, height = hgt, units = "px")
...
pc <- miles_tern %>%
  ...
  theme(plot.margin = margin(6,12,3,0,"pt"))


ga = grid.arrange(pt, pc, widths = c(3,1.85), top = grid::textGrob("NY"))

ggplot2::ggsave("NY/arrange.png", ga, width = wdt, height = hgt, units = "px")
...
df.t <- df %>%
  ...
  left_join(milesST, by = c("OPERATOR_ID", "IYEAR", "SYS"), 
             suffix = c("",".j"), keep = T)%>%
