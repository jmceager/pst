...
df.trend <- df %>%
  ...
  ungroup()%>%
  mutate(pri.name = factor(pri.name, levels = df.k$pri.name)) %>%
...
df.release %>%
  ...
  geom_label_repel(data = filter(df.release, TOTAL_RELEASE > 5 | 
                                   SIGSER == 2 & FATAL > 1 |
...
#edit these three numbers/exponents: the first to set the scene and the next two to edit the crop
xdist_exp <- 1.2*xdist
bxn <- bbox[[1]] - (xdist_exp*.7)
bxx <- bbox[[3]] + (xdist_exp*.3)
#dont edit these
...
df %>%
  ...
  summarise(NAME = paste0((unique(NAME)), collapse = "; "),
            Incidents = n()) %>% write_excel_csv("IL_Operators.csv")
```
