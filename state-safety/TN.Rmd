...
df %>%
  ...
  scale_color_manual(values = pst1,
                     ...
                     labels = c(moStateL, "Adjacent (avg.)", "U.S.A. (avg.)"))+
  scale_x_continuous(labels = seq(2010,2021,2), breaks = seq(2010,2021,2))+
  scale_y_continuous(limits = c(0, .1),breaks = seq(0,.10,.025))+ 
...
df.t <- df %>%
  left_join(milesTN, by = c("OPERATOR_ID", "IYEAR", "SYS"), 
             suffix = c("",".j"), keep = T)%>%
...
nGD <- nrow(df.t[df.t$SYS == "GD", "pri.name"])
t.gd <- df.t %>%
  filter(SYS == "GD") %>% 
  mutate(pri.name = if_else(grepl("CENTERPOINT", pri.name), "Centerpoint (GD)", pri.name),
         pri.name = fct_lump(f=pri.name, 4, w=n, other_level = paste0(nGD," Others (GD, avg.)")))%>%
  group_by(pri.name)%>%
  summarise_at(vars(n:rpi), mean, na.rm = T) %>%
  arrange(desc(n))

nGT <- nrow(df.t[df.t$SYS == "GT", "pri.name"])
t.gt <- df.t %>%
  filter(SYS == "GT") %>%
  mutate(pri.name = if_else(grepl("CENTERPOINT", pri.name), "Centerpoint (GT)", pri.name),
         pri.name = fct_lump(f=pri.name, 4, w=n, other_level = paste0(nGT," Others (GT, avg.)")))%>%
  group_by(pri.name)%>%
  summarise_at(vars(n:rpi), mean, na.rm = T) %>%
  arrange(desc(n))

nHL <- nrow(df.t[df.t$SYS == "HL", "pri.name"])
t.hl <- df.t %>%
  filter(SYS == "HL") %>% 
  mutate(pri.name = fct_lump(f=pri.name, 2, w=n, other_level = paste0(nHL," Others (HL, avg.)")))%>%
  group_by(pri.name)%>%
  summarise_at(vars(n:rpi), mean, na.rm = T)%>%
  arrange(desc(n))
...
df.trend <- df %>%
  ...
  mutate(pri.name.y = if_else(pri.name.x == "CENTERPOINT ENERGY RESOURCES CORP., DBA CENTERPOINT ENERGY MINNESOTA GAS",
         ...
         pri.name.y = if_else(is.na(pri.name.y),
                              if_else(SYS.x == "GT", paste0(nGT," Others (GT, avg.)"),
                                      if_else(SYS.x == "GD", paste0(nGD," Others (GD, avg.)"), 
                                              if_else(SYS.x == "HL", paste0(nHL," Others (HL, avg.)"),
                                                      "N"))),
                              pri.name.y),
  ...
  rbind(count(df.noinc, pri.name, SYS, IYEAR) %>% mutate(n = 0) %>% ungroup() %>%select(!OPERATOR_ID))%>%
...
df.k %>%
  ...
  group_rows(start_row = nrow(t.hl)+nrow(t.gd)+1, end_row = nrow(df.k), group_label = "Gas Transmission")%>%
...
df.release %>%
  ...
  geom_label_repel(data = filter(df.release, TOTAL_RELEASE > 2 | 
                                   SIGSER > 1 & SYS != "GD" | 
                                   SIGSER > 1 & FATAL > 0 |
                                   TOTAL_COST_CURRENT > 1000000),
                  aes(x = MDY, y = TOTAL_RELEASE, label = pri.name),
                  size = 5, color = "gray 20",  parse = F,
...
df.c%>%
  ggplot(aes(y=str_wrap(pri.name, 24), x = str_wrap(str_to_sentence(CAUSE),15), fill = cPer))+
...
df.state%>%
  ...
  theme(legend.position = c(0.015,0.975),
        ...
        plot.tag = element_text(color = "gray 30", size = 16),
        plot.title = element_text(face = "bold", colour = "black",
                                  hjust = .015, lineheight = .25),
